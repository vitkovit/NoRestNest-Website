...
- **Progression calculator** (`#engine`, section 01): a JS port of the weight+reps path of the app's `ProgressiveOverloadService` (`~/Developer/norestnest/lib/features/programs/domain/services/progressive_overload_service.dart`) — profiles, plate rounding, rep-range rules, double-progression guard, reason strings. If the app's engine changes, re-port; don't tweak numbers by hand. Bodyweight/timed paths are not ported (footnote says so).
